...
try:
    ...
    st.error("Secrets Missing: Please add GITHUB_TOKEN and GITHUB_REPO to Streamlit Secrets.")
    st.stop()
...
with st.sidebar:
    ...
    if st.button(f"🚀 Full Sync {sync_fy}"):
        with st.status(f"Extracting {sync_fy}...") as status:
            ...
            if not t_rows.empty:
                ...
                WealthEngine.push(pd.DataFrame(prices), "data/price_cache.csv")
    ...
    if f'sync_{sync_fy}' in st.session_state:
        st.caption(f"📅 **Last Sync ({sync_fy}):** {st.session_state[f'sync_{sync_fy}']}")
...
all_t, all_p = [], []
with st.expander("🛠️ System Monitor (Check for ❌)"):
    for y in years_to_load:
        t, p = WealthEngine.load(f"data/{y}/trades.csv"), WealthEngine.load(f"data/{y}/perf.csv")
        st.write(f"**{y}:** Trades {'✅' if t is not None else '❌'} | Perf {'✅' if p is not None else '❌'}")
...
if all_t:
    ...
    df_raw['Q_v'] = df_raw[c_q].apply(WealthEngine.clean_val)
    df_raw['P_v'] = df_raw[c_p].apply(WealthEngine.clean_val)
    df_raw['C_v'] = df_raw[c_c].apply(WealthEngine.clean_val).abs() if c_c else 0.0
    ...
    for ticker in df_raw[c_s].unique():
        ...
        for _, row in sym_df.iterrows():
            if row['Q_v'] > 0: open_lots.append({'dt': row['DT'], 'q': row['Q_v'], 'p': row['P_v'], 'c': row['C_v']})
            elif row['Q_v'] < 0:
                sq = abs(row['Q_v'])
    ...
    if not df_perf.empty:
        ...
        if rt_col and cat_col:
            df_perf[rt_col] = df_perf[rt_col].apply(WealthEngine.clean_val)
    ...
    st.divider()
    render_t(df_h, "1. Current Global Holdings")
    ...
    st.warning("⚠️ No data found. Open 'System Monitor' to check status, then Sync each year (FY24 first).")
